...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        const { data } = await progressApi.getCoursesProgress();
    ...
    Promise.all([fetchCourses(), fetchProgress()]).finally(() => setLoading(false));
  }, []);
  ...
  const purchasedCourses = courses.filter(course => purchasedCoursesIds.includes(course.id));
  const availableCourses = courses.filter(course => !purchasedCoursesIds.includes(course.id));
        ...
        <div className="container mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              ...
              { icon: <Clock className="w-6 h-6" />, label: 'Средний прогресс', value: `${totalProgress}%` },
              { icon: <Award className="w-6 h-6" />, label: 'Заработано сертификатов', value: '0' },
            ].map((stat, index) => (
                <motion.div
                    key={index}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {purchasedCourses.map((course, index) => (
                  <motion.div key={course.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div className="p-4">
                      <div className="flex justify-between items-center">
                        <div className="text-sm text-gray-600">
                          <span className="font-semibold">Прогресс:</span> {progressData[course.id] ?? 0}%
                        </div>
